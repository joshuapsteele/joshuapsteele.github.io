// Full Blog Post to Hugo - with metadata prompts
// This version prompts for tags, categories, and description
...
if (content && content.length > 0) {
  ...
  if (title && title.length > 0) {
    ...
    if (filename && filename.length > 0) {
      ...
      p.title = "Post Metadata";
      p.message = "Add optional metadata for your post:";
      ...
      p.addTextField("tags", "Tags", "", {
        placeholder: "Comma-separated (e.g., hugo, blogging, web)"
      });

      p.addTextField("categories", "Categories", "", {
        placeholder: "Comma-separated (e.g., Tech, Writing)"
      });

      p.addSwitch("isDraft", "Save as draft?", false);

      p.addButton("Publish");

      if (p.show()) {
        // Parse comma-separated values and format as YAML arrays
        const description = p.fieldValues["description"].trim();
        const tagsInput = p.fieldValues["tags"].trim();
        const categoriesInput = p.fieldValues["categories"].trim();
        const isDraft = p.fieldValues["isDraft"];
        ...
        if (categoriesInput) {
          ...
          if (categoriesArray.length > 0) {
            categoriesYaml = "[\"" + categoriesArray.join('", "') + "\"]";
          }
        }
...
${body}`;
        ...
        if (token) {
          ...
          if (response.statusCode === 200 || response.statusCode === 201) {
            ...
            const draftStatus = isDraft ? " (as draft)" : "";

            app.displaySuccessMessage(
              "✅ Post published" + draftStatus + "!\n\n" +
              "File: " + filename + "\n\n" +
              "It will be live at:\n" + webUrl + "\n\n" +
