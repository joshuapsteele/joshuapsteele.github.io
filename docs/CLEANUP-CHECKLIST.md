...
- [ ] **Reduce false positives in `scripts/check-internal-links.py`.** After fixing all genuinely broken internal links (see Completed), the checker still reports ~62 "broken" links that actually resolve at runtime. Three checker limitations cause this: (1) it doesn't follow Hugo `aliases` (e.g. `/uses` → `/resources`, `/tech-stack` → `/posse`); (2) it only indexes rendered HTML page directories, so generated non-HTML files (feeds like `/blog/feed.xml`, `/notes/feed.json`) and `static/` assets (`/wp-content/*.pdf`, images) are flagged as missing; (3) it folds the markdown link `"title"` text into the target URL. Building the valid-URL set by walking `public/` (which includes alias redirect pages and generated files) plus `static/`, and stripping title text before comparison, would eliminate the noise.
