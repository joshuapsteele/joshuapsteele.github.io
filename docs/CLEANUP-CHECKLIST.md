...
- [x] **Removed all sermon audio from the repo; replaced with transcripts (2026-08-24).** Cut `static/` from 437 MB to 81 MB and the built site from 494 MB to 140 MB by removing all 20 sermon audio files (356 MB). Every file was copied to `~/sermon-audio-archive/` (outside the repo, original directory structure preserved) and SHA-256 verified before deletion; the 7 generated transcripts are archived alongside them. Breakdown: 9 files (142 MB) were referenced nowhere; 1 file (47 MB, the largest on the site) was referenced only by a shortcode whose path used en-dashes where the filename used hyphens, so it never resolved; 1 file (24 MB) was shadowed by an external `stpetersbhm.org` URL in the post that nominally used it. Of the 6 posts with players, 5 already carried full manuscripts (2,000-3,100 words) and simply lost the player. The remaining 7 recordings (2.3 hours) were transcribed locally with whisper.cpp `large-v3-turbo`, reflowed into paragraphs, and published as 5 new sermon posts plus an inline transcript on the Challenge of Christmas Light post; each carries a callout disclosing that it is an automated transcript and links the original recording. `content/pages/sermons.md` now uses `READ:` links throughout, matching the rest of the page. Supersedes the 2026-05-22 "Parked legacy media cleanup" decision. `npm run build` clean (818 pages), 0 broken internal links, Amazon disclosures pass. NOTE: git history still contains the audio, so `.git` remains ~684 MB; a `git filter-repo` rewrite is a separate, unmade decision.
- [x] **Stopped the hourly full rebuild in GitHub Actions (2026-08-24).** The `schedule` trigger rebuilt and redeployed the entire site every hour to pick up Micro.blog syndication links, and shared a workflow-level `concurrency: pages` group with content pushes. Effect: pushes queued behind cron runs (observed 20 and 31 minute wall times against a 1-3 minute build) and rapid pushes cancelled each other while queued. Added a `gate` job that runs on scheduled events only, fetches `data/syndication.json`, and skips build and deploy entirely when it is unchanged; when it has changed the gate commits the refreshed file so the next run compares against a current baseline. Moved `concurrency` off the workflow onto the `deploy` job alone, so a push starts building immediately. The gate checks out at `fetch-depth: 1` (the build still needs `0` for `enableGitInfo`).
- [x] **Fixed the gate firing on every scheduled run (2026-08-24).** The first version compared `data/syndication.json` with `git diff --quiet`, but the file carries a `generated` timestamp that `fetch_syndication_links.py` rewrites on every fetch. The diff was therefore never empty: the gate fired hourly, built, deployed, and committed a timestamp-only baseline, doing strictly more work than the hourly rebuild it replaced. Confirmed in CI by the 21:24 scheduled run, which ran gate, build, and deploy. The gate now compares the JSON with `generated` removed and keys sorted, and runs `git checkout -- data/syndication.json` when only the timestamp moved so nothing is committed. Verified locally against three cases: timestamp-only change skips, a real content change builds, and a live Micro.blog fetch with no new syndication skips.
- [x] **Fixed site-wide Amazon Associates disclosures (2026-07-07).** Added the exact required statement, “As an Amazon Associate I earn from qualifying purchases,” near the start of every rendered page containing an Amazon affiliate link, plus an adjacent visible `(paid link)` label after every tagged or shortened Amazon affiliate link (including links rendered by shortcodes). Added `scripts/check-amazon-disclosures.py` as a deterministic built-site audit and wired it into both `npm run build` and GitHub Actions. Before the fix the audit found 633 disclosure failures; after the fix it verifies 108 rendered pages and all 525 rendered Amazon affiliate links. `npm run build` passes.
